...
from db import SessionLocal, DBContext, engine
...
def get_user(username: str, db: Session = None):
    if db is None:
        with DBContext() as db:
            return crud.get_user_username(db=db, username=username)
    return crud.get_user_username(db=db, username=username)

def authenticate_user(username: str, password: str, db: Session = Depends(get_db)):
    user = crud.get_user_username(db=db, username=username)
    if not user:
        return None
...
manager._not_authenticated_exception = NotAuthException
app.add_exception_handler(NotAuthException, not_authenticated_exception_handler)
...
@app.post("/tasks/delete/{id}", response_class=RedirectResponse)
...
@app.post("/login")
def login(request: Request, form_date: OAuth2PasswordRequestForm = Depends(),
          db: Session = Depends(get_db)):
    user = authenticate_user(username=form_date.username, password=form_date.password, db=db)
    if not user:
        return templates.TemplateResponse(
            "login.html", {"request": request, "title": "Login", "invalid": True},
            status_code=status.HTTP_401_UNAUTHORIZED)
    access_token_expires = timedelta(minutes=ACCESS_T_MIN)
...
def register(request: Request,
    ...
    invalid = False
    if crud.get_user_username(db=db, username=username):
        invalid = True
    ...
    if not invalid:
        ...
        return templates.TemplateResponse(
            "register.html",{"request": request, "title": "Register", "invalid": True},
...
def logout():
    ...
    manager.set_cookie(responce, None)
    return responce
